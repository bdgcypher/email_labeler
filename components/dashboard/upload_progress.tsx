...
export default function UploadProgress({setUploadInProgress, setUploadSuccess}) {
    const [progress, setProgress] = React.useState(0);

    React.useEffect(() => {
        const timer = setInterval(() => {
            setProgress((prevProgress) => (prevProgress >= 100 ? setUploadInProgress(false) & setUploadSuccess(true) : prevProgress + 10));
        }, 1000);
